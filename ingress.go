...
func isWebSocketUpgrade(r *http.Request)bool{return strings.EqualFold(r.Header.Get("Connection"),"upgrade")&&strings.EqualFold(r.Header.Get("Upgrade"),"websocket")}
func stripPort(h string)string{if host,_,err:=net.SplitHostPort(h);err==nil{return host};return h}
func flattenHeaders(h http.Header)map[string]string{out:=make(map[string]string,len(h));for k,vs:=range h{switch strings.ToLower(k){case "keep-alive","proxy-authenticate","proxy-authorization","te","trailer","transfer-encoding":continue};out[k]=strings.Join(vs,", ")};return out}
func copyHeaders(dst http.Header,src map[string]string){for k,v:=range src{switch strings.ToLower(k){case "content-length","transfer-encoding","connection":continue};dst.Set(k,v)}}
func isClosedConnErr(err error)bool{if err==nil{return false};if err==io.EOF||err==io.ErrUnexpectedEOF{return true};s:=err.Error();return strings.Contains(s,"broken pipe")||strings.Contains(s,"connection reset")}
