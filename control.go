...
func (c *controlServer) handle(conn net.Conn) {
	...
	connectorID := connectorIdentity(req.ConnectorID, req.Hostname, remote)
	resp := wire.AuthResp{OK: ok, TunnelID: tunnelID, ConnectorID: connectorID}
	if !ok {
	...
	auth.Close()
	if !ok {
		appEvents.add("warning", "connector.auth_failed", "", connectorID, "", "Connector authentication rejected from "+remote)
		sess.Close()
		return
	}
	now := time.Now().UTC()
