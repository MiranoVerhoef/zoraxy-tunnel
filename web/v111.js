...
async function requestConnectorUpdate(tunnelID,encodedConnectorID){
  ...
  else toast(data?.error||'Could not request connector update');
...
renderConnectors=function(t,c){
  ...
  const rows=list.map(cn=>{
    ...
    const updateButton=mode==='auto'&&cn.online?`<button class="btn secondary compact update-now ${busy?'updating':''}" onclick="requestConnectorUpdate('${t.id}','${encodeURIComponent(cn.id)}')">${busy?'Checking…':'↻ Update now'}</button>`:'';
